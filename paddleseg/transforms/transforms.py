...
from .tif_multiDataset import read_multiband_tif
...
def bilinear_resize(image, new_height, new_width):
    ...
    x = np.linspace(0, width - 1, num=new_width)
    y = np.linspace(0, height - 1, num=new_height)
    x_index, y_index = np.meshgrid(x, y)

    x_index = x_index.astype(np.float32)
    y_index = y_index.astype(np.float32)

    x0 = np.floor(x_index * x_scale).astype(np.int64)
    y0 = np.floor(y_index * y_scale).astype(np.int64)
    x1 = np.ceil(x_index * x_scale).astype(np.int64)
    y1 = np.ceil(y_index * y_scale).astype(np.int64)
    ...
    # 计算插值结果
    wa = (x1 - x_index * x_scale) * (y1 - y_index * y_scale)
    wb = (x_index * x_scale - x0) * (y1 - y_index * y_scale)
    wc = (x1 - x_index * x_scale) * (y_index * y_scale - y0)
    wd = (x_index * x_scale - x0) * (y_index * y_scale - y0)

    # add dimensions to wa, wb, wc, wd for broadcasting
    wa = np.expand_dims(wa, axis=2)
    wb = np.expand_dims(wb, axis=2)
    wc = np.expand_dims(wc, axis=2)
    wd = np.expand_dims(wd, axis=2)

    # Combine results of the four interpolations
    resized_image = wa * Ia + wb * Ib + wc * Ic + wd * Id

    return resized_image

# 示例使用
original_image = np.random.rand(100, 100, 3)  # 示例图像
H2, W2 = 200, 200  # 目标尺寸

# 缩放图像
resized_image = bilinear_resize(original_image, H2, W2)
# print(resized_image.shape)  # 输出: (200, 200, 3)
...
class Compose:
    ...
    def __init__(self, transforms, to_rgb=False, img_channels=3):
    ...
    def __call__(self, data):
        ...
        if isinstance(data['img'], str):
            ...
            data['img'] = read_multiband_tif(data['img'])
        ...
        if 'label' in data.keys() and isinstance(data['label'], str):
            ...
            # tif 修改
            # data['label'] = read_multiband_tif(data['label'])
            # data['label'] = np.squeeze( data['label'])
        
        if 'img2' in data.keys() and isinstance(data['img2'], str):
            # 多光谱
            # data['img2'] = read_multiband_tif(data['img2']) 
            # print(data['img2'])
            data['img2'] = np.asarray(Image.open(data['img2']))
            data['img2'] = np.stack([data['img2']]*3, axis=-1)
            # print("img2",data['img2'].shape)
            # print(data['img2'].shape)
            # data['img2'] =  bilinear_resize (data['img2'], H, W)  
            # print(data['img2'].shape)
...
class Normalize:
    ...
    def __call__(self, data):
        ...
        data['img2'] = functional.normalize(data['img2'], self.mean, self.std)
        return data
...
class RandomPaddingCrop:
    ...
    def __call__(self, data):
        ...
        if self.category_max_ratio < 1.0:
            ...
            data['img2'] = functional.crop(data['img2'], crop_coordinates)
        for key in data.get("gt_fields", []):
