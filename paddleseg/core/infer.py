...
def slide_inference(model, im,im2, crop_size, stride):
    """
...
def inference(model,
    ...
    if not is_slide:
        ...
        logit = slide_inference(model, im,im2, crop_size=crop_size, stride=stride)
    if hasattr(model, 'data_format') and model.data_format == 'NHWC':
...
def aug_inference(model,
    ...
    for scale in scales:
        ...
        im_scale2 = F.interpolate(im2, [h, w], mode='bilinear')
        for flip in flip_comb:
            im_flip2 = tensor_flip(im_scale2, flip)    
